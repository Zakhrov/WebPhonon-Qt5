...
void MainWindow::on_actionLocal_File_triggered()
{
 //this function/slot triggers a dialog box for selecting files and adds files to the tablewidget (playlist)
    int i;
    int index=sources.size();
    QStringList fnames=QFileDialog::getOpenFileNames(this,tr("Choose Files"),QStandardPaths::locate(QStandardPaths::MoviesLocation,"",QStandardPaths::LocateDirectory));
    if(!fnames.isEmpty())
